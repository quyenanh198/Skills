...
`.claude/settings.json` in this repo self-declares the plugin (`extraKnownMarketplaces` + `enabledPlugins`) so Claude Code web/cloud sessions know about it, but in practice that declaration alone wasn't enough to make it actually load — a fresh cloud session still came up without the plugin. What does reliably work is a `SessionStart` hook: `.claude/hooks/session-start.sh` runs `claude plugin marketplace add` + `claude plugin install` explicitly at container start (this is the same "setup script" mechanism Claude Code on the web prompts you to add for installing project dependencies — see [`session-start-hook`](https://code.claude.com/docs/en/cloud-environments)). One known gap: on the very first session in a brand-new container, the hook installs the plugin *during* that session's own startup, which can be too late for that session's own skill listing — a second session (or a follow-up message) in the same container picks it up correctly since the plugin is already on disk by then.
